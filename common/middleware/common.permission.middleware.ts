...
class CommonPermissionMiddleware {
        ...
        return (req: express.Request, res: express.Response, next: express.NextFunction) => {
            ...
            try {
                ...
                if (userPermissionFlags & requiredPermissionFlag) {
                    ...
                    res.status(403).send({ errors: ['Unauthorized permission flag'] });
                }
    ...
    async onlySameUserOrAdminCanDoThisAction(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (
            req.params &&
            req.params.userId &&
            parseInt(req.params.userId) === res.locals.jwt.userId
        ) {
            return next();
        } else {
            if (userPermissionFlags & PermissionFlag.ADMIN_PERMISSION) {
                ...
                return res.status(403).send({ errors: ['Not same user'] });
            }
        }
    }
